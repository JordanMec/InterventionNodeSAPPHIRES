...
if nargin < 2 || isempty(envData)
    useExternalEnv = false;     % legacy sinusoid
    total_time = 21600;         % [s] fallback: 6-hour run (as original)
else
    useExternalEnv = true;
    ...
    % Build per-second look-ups
    total_time = height(envData) * 3600;   % 1 row = 1 hour
    

    pm10Hourly    = envData.PM10;             % [g/m]
    tempCHourly   = envData.TempC;            % [C]

    % Repeat each hourly value for 3600 time-steps
    ...
    baseline_PM_ts= pm10Series(:) .* pm10Dist;  % [num_steps6]
end
%% ------------------------------------------------------------------------
 ...
 dust_bins               = zeros(6, num_steps);   % 6 size bins for dust tracking
...
rho_outdoor = Patm / (R_air * T_outdoor_K);  % [kg/m³]
 ...
 % =========================================================================
     ...
     energy_kWh = (current_power / 1000) * (dt/3600); % now = 1 hr, so dt/3600 = 1
     ...
     if Q_blower > 0
        ...
        T_out_C = T_outdoor_C;            % <-- changed to account for C
 ...
 % =========================================================================
    ...
    if useExternalEnv
    C_out_PM = baseline_PM_ts(cs,:);    % [16] from file
else
    C_out_PM = baseline_PM_ts(cs,:);    % legacy
end
...
% Dust mass-flux captured by HEPA (6 size bins for dust bins, different from PM bins)
conc_bin = [4000, 8000, 5000, 1000, 1500, 200] * 1e-6;  % example g/m per dust bin (6 bins)
eff_bin  = [0.90, 0.85, 0.95, 0.99, 0.999, 0.9999];       % HEPA capture efficiency for 6 bins
     ...
     dust = dust + flux_bin * dt;  % now dt = 3600 s → dust captured per hour
 ...
 legend('Bin 1', 'Bin 2', 'Bin 3', 'Bin 4', 'Bin 5', 'Bin 6', 'Location', 'best');
 grid on;
 ...
 for i = 1:numSizes
    ...
    yline(baseline_PM_ts(1,i),'r--','LineWidth',1.0);
     ...
     xlabel('Time (h)');
     ylabel(sprintf('PM %.1f m (g/m)', particle_sizes(i)));
     if i == 1
 ...
 results.C_indoor_PM         = C_indoor_PM;            % [g/m] indoor PM per bin
 ...
 results.total_PM10          = sum(C_indoor_PM, 2);    % [g/m] summed over all bins
 ...
 % Optional: Automatically append this runs metadata to a cumulative table
